...
public class UI_GameBuildController : MonoBehaviour
{
    ...
    {
        UpdateAvailableChallengesForMode(gameData.Mode);

        var required = new List<Required>();

        for (int i = 0; i < gameData.Mode.Games.Count; i++)
        {
            if (gameData.Games.Count == i) gameData.Games.Add(new BuildGame());

            if (!gameData.Mode.AllowChallengeSelection) continue;

            required.Clear();
            required.AddRange(BuildRequiaredList(gameData.Mode.Games[i].RequiaredChallenges));
            ...
            gameData.Games[i].Challenge = selected;
    ...
    protected virtual BuildGameData GenerateVillains(BuildGameData gameData)
    {
        ...
        {
            required.Clear();
            required.AddRange(BuildRequiaredList(gameData.Mode.Games[i].RequiaredVillains));
            if (gameData.Games[i].Challenge != null) required.AddRange(BuildRequiaredList(gameData.Games[i].Challenge.GameDtl.RequiaredVillains));

            var numberOfVillainsForGame = gameData.Mode.Games[i].NumberOfVillains;
            ...
            for (int j = 0; j < numberOfVillainsForGame; j++)
            {
                ...
                if ((!IsVillainValid(gameData.Mode.Games[i], gameData.Games[i], selected)) || (!availableVillainsForGame.Contains(selected)))
                {
    ...
    protected virtual void UpdateVillainsForModeAndChallenege(BuildGameData gameData)
    {
        for (int i = 0; i < gameData.Mode.Games.Count; i++)
        {
            if (gameData.Mode.Games[i].RequiaredVillains.Count > 0)
            {
                foreach(var vill in gameData.Mode.Games[i].RequiaredVillains)
                {
                    foreach (Character obj in vill.RequiredObjects)
                    {
            ...
            if (gameData.Games[i].Challenge != null && gameData.Games[i].Challenge.GameDtl.RequiaredVillains.Count > 0)
            {
                foreach (var vill in gameData.Games[i].Challenge.GameDtl.RequiaredVillains)
                {
                    foreach (Character obj in vill.RequiredObjects)
                    {
                        if (!availableVillains.Contains(obj)) availableVillains.Add(obj);
    ...
    protected virtual bool IsVillainValid(GameDtl modeDtl, BuildGame gameData, Character villain)
    {
        ...
        if (gameData.Challenge != null && gameData.Challenge.GameDtl.ExcludedVillains.Contains(villain)) return false;
    ...
    protected virtual BuildGameData GenerateLocations(BuildGameData gameData)
    {
        ...
        for (int i = 0; i < gameData.Mode.Games.Count; i++)
        {
            ...
            required.Clear();
            required.AddRange(BuildRequiaredList(gameData.Mode.Games[i].RequiaredLocations));
            if (gameData.Games[i].Challenge != null) required.AddRange(BuildRequiaredList(gameData.Games[i].Challenge.GameDtl.RequiaredLocations));
            ...
            for (int j = 0; j < numberOfLocations; j++)
            {
                ...
                if ((selected.ExclusiveForMode.Count > 0 && !selected.ExclusiveForMode.Contains(gameData.Mode.ModeTag)) || 
                    (selected.ExclusiveForMode.Count > 0 && selected.ExclusiveForMode.Contains(gameData.Mode.ModeTag)  && !RequiredContains(required, selected)))
                {
                ...
                if (!IsLocationValid(gameData.Mode.Games[i], gameData.Games[i], selected) || (!availableLocationsForGame.Contains(selected)) || gameData.Games[i].Locations.Contains(selected))
                {
                    availableLocationsForGame.Remove(selected);
                    if (required.Count > 0) UpdateRequired(required, selected, false);
                    j--;
    ...
    protected virtual bool IsLocationValid(GameDtl modeDtl, BuildGame gameData, Location location)
    {
        ...
        //check if valid for challenege
        if (gameData.Challenge != null && gameData.Challenge.GameDtl.ExcludedLocations.Contains(location)) return false;
    ...
    protected virtual BuildGameData GenerateHeros(BuildGameData gameData)
    {
        ...
        for (int i = 0; i < gameData.Mode.Games.Count; i++)
        {
            ...
            if (i > 0 && gameData.Mode.UseSameHerosForAllGames)
            {
            ...
            if (gameData.Games[i].Challenge != null) required.AddRange(BuildRequiaredList(gameData.Games[i].Challenge.GameDtl.RequiredHeros));
            foreach (var villain in gameData.Games[i].Villains)
            {
            ...
            for (int j = 0; j <= NumberOfHerosDropDown.value; j++)
            {
                ...
                if ((!IsHeroValid(gameData.Mode.Games[i], gameData.Games[i], selected)) || (!availableHerosForGame.Contains(selected)))
                {
    ...
    protected virtual bool IsHeroValid(GameDtl modeDtl, BuildGame gameData, Character hero)
    {
        ...
        //check if valid for challenege
        if (gameData.Challenge != null && gameData.Challenge.GameDtl.ExcludedHeros.Contains(hero)) return false;
